...
describe("authService", () => {
	...
	describe("refreshTokens", () => {
		it("有効なリフレッシュトークンで新しいトークンペアを生成できること", async () => {
			...
			vi.spyOn(tokenBlacklistRepository, "isTokenBlacklisted").mockResolvedValueOnce({
				ok: true,
				value: false,
			});
			...
			expect(userRepository.findUserById).toHaveBeenCalledWith(userId, mockPrisma);
		});
	...
	describe("logout", () => {
		it("トークンを正常にブラックリストに追加できること", async () => {
			...
			vi.spyOn(tokenBlacklistRepository, "addToBlacklist").mockResolvedValueOnce({
				ok: true,
				value: undefined,
			});
